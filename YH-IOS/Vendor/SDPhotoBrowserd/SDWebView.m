...
@property (nonatomic, strong) WKUserScript* heightUserScript;
@end
...
- (instancetype)initWithURLString:(NSString *)urlString {
    self = [self initWithFrame:CGRectZero configuration:[WKWebViewConfiguration new]];
    [self setURLString:urlString];
    return self;
}

- (instancetype)initWithFrame:(CGRect)frame {
    self = [self initWithFrame:frame configuration:[WKWebViewConfiguration new]];
    return self;
}
...
- (void)setURLString:(NSString *)URLString {
    ...
    if (URLString) {
        [self loadRequest:[[NSURLRequest alloc] initWithURL:[[NSURL alloc] initWithString:URLString]]];
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if ([keyPath isEqualToString:@"title"]) {
        // 判断是哪个对象
        if (object == self) {
            self.webViewtitle = self.title;
            __weak typeof(self) weakSelf = self;
            if (weakSelf.titleBlock) {
                weakSelf.titleBlock(weakSelf.title);
            }
        }
    }
}
...
- (void)webView:(WKWebView *)webView runJavaScriptAlertPanelWithMessage:(NSString *)message initiatedByFrame:(WKFrameInfo *)frame completionHandler:(void (^)(void))completionHandler{
    if (_webDelegate && [_webDelegate respondsToSelector:@selector(webView:runJavaScriptAlertPanelWithMessage:initiatedByFrame:completionHandler:)]) {
        [_webDelegate webView:webView runJavaScriptAlertPanelWithMessage:message initiatedByFrame:frame completionHandler:completionHandler];
    }
}
...
- (WKUserScript *)heightUserScript{
    if (!_heightUserScript) {
        NSString *jsString = [NSString stringWithFormat:@"<html> \n"
                                     "<head> \n"
                                     "<style type=\"text/css\"> \n"
                                     "img {max-width:%fpx;} *{font-size:0.98rem;color:#777777} body{width:%fpx;position:relative;margin-right:10px;word-break: break-all;word-wrap: break-word;height:auto;}\n"
                                     ...
                                     "</html>",([UIScreen mainScreen].bounds.size.width-20),[UIScreen mainScreen].bounds.size.width*0.96 , self.des];
        _heightUserScript = [[WKUserScript alloc] initWithSource:jsString injectionTime:WKUserScriptInjectionTimeAtDocumentEnd forMainFrameOnly:false];
        [self.configuration.userContentController addUserScript:_heightUserScript];
    }
    return _heightUserScript;
}
