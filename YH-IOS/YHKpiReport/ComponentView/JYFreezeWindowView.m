...
@implementation JYFreezeWindowView
...
- (instancetype)initWithFrame:(CGRect)frame FreezePoint: (CGPoint) freezePoint cellViewSize: (CGSize) cellViewSize withSizeArray:(NSArray *)sizeArray {
    ...
    if (self) {
        ...
        _signView = [[JYSignView alloc] initWithFrame:CGRectMake(0, 0, freezePoint.x, freezePoint.y)];
...
- (JYMainViewCell *)dequeueReusableMainCellWithIdentifier:(NSString *)identifier forIndexPath:(NSIndexPath *)indexPath {
    ...
    JYMainViewCell *mainViewCell = [mainCellsWithIndexPath objectForKey:indexPath];
    return mainViewCell;
}
...
- (JYSectionViewCell *)dequeueReusableSectionCellWithIdentifier:(NSString *)identifier forSection:(NSInteger)section {
    ...
    JYSectionViewCell *sectionViewCell = [sectionCellsWithSection objectForKey:[NSString stringWithFormat:@"%ld",(long)section]];
...
- (JYRowViewCell *)dequeueReusableRowCellWithIdentifier:(NSString *)identifier forRow:(NSInteger)row {
    ...
    JYRowViewCell *rowViewCell = [rowCellsWithRow objectForKey:[NSString stringWithFormat:@"%ld",(long)row]];
    return rowViewCell;
}
...
- (void)scrollViewDidScroll:(nonnull UIScrollView *)scrollView
{
    ...
    if ([scrollView isEqual:self.mainScrollView]) {
        ...
        if (self.bounceStyle == JYFreezeWindowViewBounceStyleAll) {
        ...
        self.mainScrollView.delegate = nil;
        self.sectionScrollView.delegate = nil;
        if (self.bounceStyle == JYFreezeWindowViewBounceStyleAll && self.rowScrollView.contentOffset.y <= 0) {
...
- (void)sectionCellInSectionScrollView {
    if (_delegate) {
        if (self.keyIndexPath != nil) {
            JYSectionViewCell *sectionViewCell = [_delegate sectionCellPointInfreezeWindowView:self];
...
- (void)setDataSource:(id<JYFreezeWindowViewDataSource> __nullable)dataSource {
...
- (void)setDelegate:(id<JYFreezeWindowViewDelegate> __nullable)delegate {
    if (_delegate != delegate) {
        _delegate = delegate;
    }
}

- (void)setStyle:(JYFreezeWindowViewStyle)style {
    _style = style;
}

- (void)setBounceStyle:(JYFreezeWindowViewBounceStyle)bounceStyle {
    _bounceStyle = bounceStyle;
    switch (bounceStyle) {
        case JYFreezeWindowViewBounceStyleNone:
        {
            ...
            break;
        case JYFreezeWindowViewBounceStyleMain:
        {
            self.mainScrollView.bounces = YES;
            self.sectionScrollView.bounces = YES;
            self.rowScrollView.bounces = YES;
        }
        case JYFreezeWindowViewBounceStyleAll:
        {
            self.mainScrollView.bounces = YES;
            self.sectionScrollView.bounces = YES;
            self.rowScrollView.bounces = YES;
...
- (void)removeMainViewCellWithIndexPath:(NSIndexPath *)indexPath {
    JYMainViewCell *mainViewCell = [_dataSource freezeWindowView:self cellForRowAtIndexPath:indexPath];
...
- (void)removeRowViewCellWithRow:(NSInteger)row {
    JYRowViewCell *rowViewCell = [_dataSource freezeWindowView:self cellAtRow:row];
        ...
        if ([self dequeueReusableRowCellWithIdentifier:rowReuseIdentifier forRow:row] == nil) {
            ...
            switch (_style) {
                ...
                case JYFreezeWindowViewStyleRowOnLine:
                    ...
                    rowViewCell.separatorStyle = JYFreezeViewCellSeparatorStyleNone;
                default:
                    break;
            }
