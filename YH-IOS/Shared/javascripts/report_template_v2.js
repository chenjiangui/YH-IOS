...
(function(){
  ...
  window.ReportTemplateV2 = {
    ...
    generateTableDataV3: function(heads, rows, tabIndex) {
      var tableData = {headColumns:[], dataColumns:[], columnDefs: []}, rowmaxlen = {};
      ...
      for(var index = 0, len = heads.length; index < len; index ++) {
        var maxlen = rowmaxlen[index] , headlen = ReportTemplateV2.getLength(heads[index]);
    ...
    setSearchItems: function() {
      var items = [], constructed_items, max_deep = 0, name_parts = [];
      for(var i = 0, len = window.TemplateDatas.length; i < len; i++) {
    ...
    setSearchItemsV2: function() {
      var items = [], constructed_items, max_deep = 0, name_parts = [], first_name = null;
      for(var i = 0, len = window.TemplateDatas.length; i < len; i++) {
        if(typeof(window.TemplateDatas[i].name) === 'string') {
    ...
    databargtablesfunc : function(tabIndex, head, rows, columnDefs, displaypercent) {
         ReportTemplateV2.bargtableIds[tabIndex] = $('#table_'+tabIndex).DataTable({
              ...
              createdRow: function (row, data, index) {
                  ...
                  if(barMax <= 0) {  barlW = (Math.abs(barMin)/barLen) * 100; barrW = 0; leftbkg = ' leftbkg';   }
                  if(barMin >= 0) {  barlW = 0; barrW = ((barMax)/barLen) * 100; rightbkg = ' rightbkg'; }
                  if(barMax > 0 && barMin < 0) {
                      if(Math.abs(barMax) > Math.abs(barMin)) {
                         barlW = (Math.abs(barMin)/barLen) * 100; barrW = ((barMax)/barLen) * 100; rightbkg = ' rightbkg';
                      } else {
                         barlW = (Math.abs(barMin)/barLen) * 100; barrW = (barMax/barLen) * 100;  leftbkg = ' leftbkg';
                      }
                  }
...
$(function() {
 ...
 ReportTemplateV2.setSearchItemsV2();
