...
@interface TemplateSixViewController ()<UITableViewDelegate,UITableViewDataSource,UIPopoverPresentationControllerDelegate,DropViewDelegate,DropViewDataSource,UIWebViewDelegate,CLLocationManagerDelegate,UINavigationControllerDelegate,UINavigationBarDelegate,AMapLocationManagerDelegate>
{
...
@implementation TemplateSixViewController
...
-(void)addNavigationView{
    ...
    self.rightItem = [[UIBarButtonItem alloc]initWithImage:[[UIImage imageNamed:@"btn_add"]imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] style:UIBarButtonItemStylePlain target:self action:@selector(onRightBtn:)];
...
- (void)addWebViewJavascriptBridge {
    ...
    // 注册原生报表
    [self.bridge registerHandler:@"callRealTimeReportMenu" handler:^(id data, WVJBResponseCallback responseCallback) {
        NSLog(@"%@",data);
        if (data[@"params"] !=nil && ![data[@"params"] isEqualToString:@""])  {
